...
data <- read.csv("X:/Agency_Files/Outcomes/Risk_Eval_Air_Mod/_Air_Risk_Evaluation/R/R_Camp/Intro to R\RTrain - Star Wars/data/starwars_scrap_jakku_full.csv")
```
...
> Wouldn't it be nice if we could easily find the mean price for every Origin City? It's time for `group_by()`!
...
#### <i class="fa fa-user-starwars" aria-hidden="true" style="color: green"></i> Exercise  {-}
<br>
...
```{r, eval=F}
group_by(scrap, Dest) %>% 
  mutate(median_weight = _________________)
```
...
```{r, eval=F}
group_by(scrap, Dest) %>% 
  mutate(median_weight = median(Ton_Conv, na.rm = T))
```
