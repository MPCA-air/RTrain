...
<div class="well">
<h3><i class="fas fa-mitten"></i></h3>

Are there any questions? Any questions at all. No matter how large or terribly small... Last chance, we are about to move on...
</div>

<br>

![](images/moving_on.jpg){width="50%" style="margin-left: 7%; margin-bottom: 10px;"}
...
![](https://itep-r.netlify.com/images/planet_endor.jpg){align="center" style="width: 62%; margin-top: -4px;"}

<br>

__Let's get acquainted with our new Endor data set.__ Remember the different ways?
...
mag_endor <- filter(mag_endor, year(date) == "2017")
```
...
ggplot(mag_endor, aes(x = week_day, y = week_month, fill = Result)) +
  geom_tile(color = "gray") +
  facet_wrap(~month_text) +
  geom_label(aes(label = mday(date)), color = "white", size = 8)
...
### ...I wish I could type the data frame name less often. {-}

You can! You can chain functions together to make your script more streamlined by using the `%>%`. Also known as the _pipe operator_.
...
Here are two ways the `%>%` operator is helpful. First, it eliminates the need for nested parentheses. 
...
The code doesn't look much like the words we used to describe it. Let's see if it's easier to read when we pipe one function to another.
...
Second, it eliminates the need to and refer to the data frame each time you create a new line to apply a function to your data.

<br>

Here's all of the functions we have applied to the scrap data so far:
...
Now, let's use the `%>%` operator and write it this way.

```{r review_better, eval = F}
...
Make the Endor air data scripts more efficient by using the pipe.
