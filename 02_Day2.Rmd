...
<br>
  ...
  - First we made a wordcloud with the package wordcloud2. 
  - This exercise taught us to write in command line and how to install and open packages in R Studio.
  - And it was beautiful!
  
  <br>
  

![](images/name_cloud_mpca.png){style="width: 100%; margin-top: 12px;"}
...
<br>
  ...
  - Next we took a tour of R Studio.
  - We took a tour of R Studio.
  - Are there are any remaining questions about that?
...
<br>
  ...
  - Later we learned about objects, the assignment operator, and how to remove objects from your environment.

  - We made lists with `c()`

  - Then, big dramatic pause here, we made a table known as a data.frame.
...
<br>
  ...
  - And then we made our first plot with the `ggplot()` function using the *ggplot2 package*.
...
<br>
  ...
  - Any questions at all..no matter how large or how small...

  - Ask them now, because we are moving on...
  

<br>

![](images/moving_on.jpg){align="left" width="400" style="margin-right:18px; margin-bottom: 12px;"}
...
### Your new analysis toolbox <i class="fa fa-toolbox" aria-hidden="true" style="color: darkgray"></i> {-}
...
| `arrange()`   | Reorder or sort rows by the values of a column      |  
| `filter()`    | Keep only certain rows based on the values of a column | 
...
## <span style="text-decoration: underline;"> The Rules </span> {-}
...
![](images/endor_forest.jpg){width="500"}
...
![](images/endor_planet.png){align="center" style="width: 62%; margin-top: -4px;"}
...
We will use `ggplot()` and bring out the `geom_tile()` to organize our data by dates and find when adn if that storm happened.

First we will pull more information out of those dates.



```{r, calendar_dates, fig.width=13, fig.height=7, out.width=1000}

mag_endor <- mutate(mag_endor, day = wday(date, label = TRUE), month = month(date), month_text = month(date, label = TRUE), year = year(date))
...
ggplot(mag_endor, aes(date, reorder(month_text, -month), fill = Result)) +
  geom_tile(color = "gray") +
  facet_wrap(~year) +
  geom_label(aes(label = mday(date)), colour = "white", size = 8) +
  ylab(NULL)
...
It looks like there really were high magnetic field readings in November of Galactic year 2017. With luck, the Resistance will reward us handsomely for this information.
